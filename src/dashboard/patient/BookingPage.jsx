                ...
                (d) => (
                  <div key={d.id} className="card">
                    <div className="docL">
                      ...
                      <div>
                        <strong>
                          <button className="link" onClick={() => nav(`/patient-dashboard/doctor/${d.id}`)}>
                            {d.name}
                          </button>
                        </strong>
                    ...
                    <div className="actions">
                      ...
                      <AmbulanceButton
                        labelDefault="Book"
                        labelSuccess="Booked"
                        onComplete={() => setDoctorModal({ doctor: d, hospitalId: selectedHospital })}
                        size="sm"
                      />
                    </div>
                  </div>
      ...
      {doctorModal && (
        <div className="modal">
          <div className="modalCard" role="dialog" aria-modal="true">
            ...
            <div className="muted">
              {doctorModal.doctor.spec || "General"} • {doctorModal.doctor.exp || 10} yrs exp
            </div>
            <div className="muted" style={{ marginTop: 4 }}>
              Rating ⭐ {doctorModal.doctor.rating || 4.5}
            </div>
            ...
            <div className="slots">
              <div className="days">
                {days.map((d) => (
                  <button
                    key={d.key}
                    className={`chip ${selectedSlot.startsWith(d.key) ? "on" : ""}`}
                    onClick={() => setSelectedSlot(`${d.key} 09:00`)}
                  >
                    {d.label}
                  </button>
                ))}
              ...
              <div className="times">
                {times.map((t) => (
                  <button
                    key={t}
                    className={`chip ${selectedSlot.endsWith(" " + t) ? "on" : ""}`}
                    onClick={() => {
